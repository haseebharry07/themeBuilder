...
(function () {
  // Inject override CSS early to neutralize CRM loader
  const override = document.createElement("style");
  override.textContent = `
    /* 🔥 Force-hide or neutralize default CRM loader styles */
    #app + .app-loader {
      all: unset !important;
  ...
  // 🌀 Wait until CRM loader element appears
  const checkLoader = setInterval(() => {
    const crmLoader = document.querySelector("#app + .app-loader");
    if (crmLoader) {
      clearInterval(checkLoader);

      // Inject your custom loader inside CRM loader
      crmLoader.innerHTML = `
      ...
      window.addEventListener("load", () => {
        crmLoader.classList.add("fade-out");
        setTimeout(() => {
          crmLoader.remove();
        }, 600);
      });
